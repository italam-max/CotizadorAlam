...
import { 
  ...
  BarChart, Activity, FileText, FolderOpen, Sparkles, Package
} from 'lucide-react';
...
interface SidebarProps {
  ...
  userProfile: UserProfile | null; // <--- Necesario para el permiso
}

export function Sidebar({ currentView, setView, onNewQuote, quotes, onSelectQuote, userProfile }: SidebarProps) {
  
  const recentQuotes = quotes.slice(0, 5);
  
  // Verificamos si tiene permisos de Admin o Soporte
    ...
    return (
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all duration-500 group relative overflow-hidden ${
          isActive
            ? 'bg-gradient-to-r from-[#D4AF37] via-[#FBBF24] to-[#D4AF37] text-[#0A2463] shadow-[0_0_20px_rgba(212,175,55,0.4)] translate-x-1' 
            : 'text-white/70 hover:text-white hover:bg-white/10 hover:shadow-inner hover:translate-x-1' 
        }`}
      >
        {isActive && (
    ...
    <aside className="hidden md:flex flex-col w-64 h-full relative overflow-hidden z-20 print:hidden border-r-2 border-[#D4AF37] shadow-[5px_0_30px_-5px_rgba(0,0,0,0.5)]">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-8 scrollbar-thin scrollbar-thumb-[#D4AF37]/30 scrollbar-track-transparent">
            ...
            {/* GRUPO ADMINISTRACIÓN (SOLO ADMIN/SOPORTE) */}
            {isAdmin && (
                <div className="space-y-1">
                    <div className="px-4 flex items-center gap-2 mb-2 opacity-90">
                        <Sparkles size={10} className="text-[#D4AF37]"/>
                        <p className="text-[10px] font-black text-[#D4AF37] uppercase tracking-[0.2em]">Gestión</p>
                    </div>
                    {/* ENLACE AL CATÁLOGO MAESTRO */}
                    <MenuLink id="catalog" icon={Package} label="Catálogo Maestro" />
                </div>
            )}
