...
import ProductCatalogManager from './features/admin/ProductCatalogManager'; // <--- Importamos el componente
...
export default function ElevatorQuoter() {
  ...
  // Agregamos 'catalog' a los tipos de vista permitidos
  const [view, setView] = useState<'dashboard' | 'quoter' | 'quotes-list' | 'ticket' | 'traffic-tool' | 'planner' | 'preview' | 'ops-calculator' | 'tracker' | 'admin' | 'catalog'>('dashboard');
  ...
  return (
    <ConfigProvider>
            ...
            <div className="max-w-[1920px] w-full mx-auto flex justify-between items-center relative z-10">
                <div className="flex items-center gap-6 cursor-pointer group" onClick={() => setView('dashboard')}>
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-[#D4AF37] font-bold tracking-[0.3em] uppercase mt-1.5" style={{ textShadow: '0 0 10px rgba(212, 175, 55, 0.5)' }}>Ascending Together</span>
                    </div>
                </div>
                ...
                <div className="hidden xl:flex flex-col items-center justify-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <h2 className="text-2xl font-black text-white tracking-[0.15em] uppercase flex items-center gap-4 transition-all hover:scale-105" style={{ textShadow: '0 0 20px rgba(255, 255, 255, 0.1)' }}>
        ...
        <div className="flex-1 flex overflow-hidden relative z-10">
            ...
            {/* VISTA CATÁLOGO (Nueva) */}
            {view === 'catalog' && (
                <div className="flex-1 overflow-y-auto bg-[#F9F7F2]/30">
                    <ProductCatalogManager isEmbedded={false} onNotify={showNotify} />
                </div>
            )}
